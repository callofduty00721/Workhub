...
export const listStartups = asyncHandler(async (req, res) => {
  const { search, industry, subIndustry, stage, sort = "newest", page = 1, limit = 12 } = req.query;
...
export const updateStartup = asyncHandler(async (req, res) => {
  const startup = await Startup.findById(req.params.id);
  if (!startup) throw new ApiError(404, "Startup not found");
  if (startup.founder.toString() !== req.user._id.toString() && req.user.role !== "super_admin") {
    throw new ApiError(403, "You do not have permission to edit this startup");
  }
...
export const deleteStartup = asyncHandler(async (req, res) => {
  ...
  if (!startup) throw new ApiError(404, "Startup not found");
  if (startup.founder.toString() !== req.user._id.toString() && req.user.role !== "super_admin") {
    throw new ApiError(403, "You do not have permission to delete this startup");
  }
...
export const submitVerificationRequest = asyncHandler(async (req, res) => {
  ...
  if (startup.founder.toString() !== req.user._id.toString()) {
    throw new ApiError(403, "Only the founder can request verification for this startup");
  }
