...
import ContestEntry from "./contestEntry.model.js";
...
// Categories are free text on Contest (clients type their own when posting,
// unlike the fixed SERVICE_CATEGORIES taxonomy on Freelancer/Service/Project)
// — so the browse filter offers whatever categories real open contests
// actually have, rather than a list that might not match anything.
export const getContestCategories = asyncHandler(async (req, res) => {
  const categories = await Contest.distinct("category", { status: "open" });
