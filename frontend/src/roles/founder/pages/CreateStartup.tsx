          ...
          <form id="create-startup-form" onSubmit={handleSubmit(onSubmit)} className="mt-6 grid gap-5 lg:grid-cols-[260px_1fr_280px] lg:items-start" noValidate>
                    ...
                    >
                      <span
                        className={cn(
                          ...
                          step === s.id ? "bg-[#2563eb] text-white" : step > s.id ? "bg-[#FF5722] text-white" : "bg-[#f1f5f9] text-[#64748b]"
                        )}
                      >
            ...
            <div className="min-w-0 rounded-2xl border border-[#e2e8f0] bg-white p-6">
              ...
              {STAGE_TIPS[watch("stage")]?.[step as (typeof STEPS)[number]["id"]] && (
                <div className="mt-3 flex items-start gap-2 rounded-xl border border-[#e8effe] bg-[#f5f8fe] px-3.5 py-2.5">
                  <Info className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#2563eb]" />
                  <p className="text-[12px] leading-relaxed text-[#334155]">{STAGE_TIPS[watch("stage")][step as (typeof STEPS)[number]["id"]]}</p>
                </div>
              )}
                ...
                <ul className="mt-3 space-y-2">
                  {["Be clear and concise", "Use simple and professional language", "Add accurate and honest information", "Proper details increase trust"].map((g, i) => (
                    <li key={i} className="flex items-start gap-2 text-[12px] text-[#334155]">
                      <Check className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#FF5722]" /> {g}
                    </li>
                  ))}
                </ul>
              </div>
