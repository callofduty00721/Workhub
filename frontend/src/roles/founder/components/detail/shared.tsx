...
export function fileIconFor(url: string) {
  ...
  if (["xls", "xlsx", "csv"].includes(ext)) return { icon: FileSpreadsheet, bg: "#F5F5F5", fg: "#171717" };
  if (["zip", "rar", "7z"].includes(ext)) return { icon: Archive, bg: "#F5F5F5", fg: "#171717" };
  if (["doc", "docx"].includes(ext)) return { icon: FileText, bg: "#F5F5F5", fg: "#171717" };
...
export const UPDATE_CATEGORY_META: Record<string, { bg: string; fg: string; icon: LucideIcon }> = {
  Announcement: { bg: "#F5F5F5", fg: "#171717", icon: Megaphone },
  Milestone: { bg: "#fdf1de", fg: "#d97706", icon: Flag },
  Partnership: { bg: "#f1ebfc", fg: "#7c3aed", icon: Handshake },
  Development: { bg: "#F5F5F5", fg: "#171717", icon: Settings },
...
export function EmptyNote({ text }: { text: string }) {
  return <p className="text-[12.5px] text-muted-foreground/70">{text}</p>;
}
...
export function DocRow({ name, url }: { name: string; url: string }) {
  return (
    <div className="flex items-center gap-3 py-3">
      ...
      <p className="flex-1 truncate text-[13px] font-semibold text-foreground">{name}</p>
      <a href={url} target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded-lg border border-border px-2.5 py-1.5 text-[11.5px] font-bold text-foreground hover:bg-muted">
