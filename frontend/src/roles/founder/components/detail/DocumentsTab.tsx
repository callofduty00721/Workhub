  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="text-[19px] font-extrabold text-foreground">Documents</h3>
          <p className="mt-1 text-[12.5px] text-muted-foreground">Important documents and files related to this startup.</p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-border px-3.5 py-2 text-[12.5px] font-bold text-foreground hover:bg-muted"
          >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          ...
          className={cn("rounded-full px-3 py-1.5 text-[11.5px] font-semibold", category === "All Documents" ? "bg-brand text-brand-foreground" : "text-muted-foreground hover:bg-muted")}
        >
          All Documents
        </button>
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-border">
        {filtered.length === 0 ? (
          ...
          <table className="w-full text-left text-[12.5px]">
            <thead>
              <tr className="border-b border-border bg-muted text-[11px] uppercase tracking-wide text-muted-foreground/70">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                          ...
                          {d.description && <p className="text-[11px] text-muted-foreground/70">{d.description}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{d.category}</td>
                    <td className="px-4 py-3 text-muted-foreground">{d.uploadedAt ? new Date(d.uploadedAt).toLocaleDateString(undefined, { day: "2-digit", month: "short", year: "numeric" }) : "—"}</td>
                    <td className="px-4 py-3 text-muted-foreground">{d.fileSize || "—"}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <a href={d.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded-lg border border-border px-2.5 py-1.5 text-[11.5px] font-bold text-foreground hover:bg-muted">
                          <Eye className="h-3.5 w-3.5" /> View
                        </a>
                        <a href={d.url} download target="_blank" rel="noreferrer" className="flex items-center gap-1 rounded-lg border border-border px-2.5 py-1.5 text-[11.5px] font-bold text-foreground hover:bg-muted">
