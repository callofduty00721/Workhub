...
import { Wallet, Briefcase, FolderKanban, Trophy, Lock, Unlock, ArrowDownToLine, Settings, Loader2, ChevronDown, ChevronUp, Receipt } from "lucide-react";
        ...
        <div className="space-y-6">
          <Card className="border-primary/30 bg-primary/5">
            <CardContent className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                variant="gradient"
                disabled={!wallet || wallet.availableBalance <= 0 || user?.kycStatus !== "verified"}
          ...
          {user?.kycStatus !== "verified" && (
            <Card className="border-warning/30 bg-warning/5">
              <CardContent className="flex items-center gap-3 p-4 text-sm">
                <Settings className="h-4 w-4 shrink-0 text-warning" />
                <p>
                  {user?.kycStatus === "pending"
                    ? "Your identity verification is under review — you'll be able to withdraw once it's approved."
                    : user?.kycStatus === "rejected"
                      ? "Your verification was rejected. Please resubmit your documents to enable withdrawals."
                      : "Complete identity verification (KYC) before you can withdraw your earnings."}{" "}
                  {user?.kycStatus !== "pending" && (
                    <Link to="/dashboard/profile" className="font-medium text-primary hover:underline">
                      Verify now
                    </Link>
                  )}
                </p>
