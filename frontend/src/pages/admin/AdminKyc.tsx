...
export default function AdminKyc() {
  ...
  return (
    <DashboardLayout role="super_admin" title="KYC Verification Requests" subtitle="Review freelancer identity documents before they can withdraw earnings.">
      {isLoading ? (
        ...
        <Card className="flex flex-col items-center gap-2 py-16 text-center">
          <p className="text-sm font-medium">No pending KYC requests</p>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Submitted {new Date(req.kycSubmittedAt).toLocaleDateString()}</p>
                  </div>
                </div>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {resolving?.action === "approve"
                ? `${resolving.request.name} will be able to withdraw their earnings.`
                : "The freelancer will be notified and can resubmit with clearer documents."}
            </DialogDescription>
          </DialogHeader>
