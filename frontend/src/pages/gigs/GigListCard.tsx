...
export function GigListCard({ service }: { service: Service }) {
  ...
  return (
    <Link to={`/services/${service._id}`} className="block">
      <div className="group overflow-hidden rounded-[28px] border border-neutral-200 bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-all duration-300 hover:border-neutral-300 hover:shadow-[0_20px_40px_-20px_rgba(15,23,42,0.18)] sm:p-5">
        <div className="grid gap-5 sm:grid-cols-[minmax(0,340px)_1fr]">
          ...
          <div className="flex w-full shrink-0 flex-col gap-2.5">
            <div className="relative aspect-[16/10] w-full overflow-hidden rounded-2xl bg-neutral-100">
              {images.length > 0 ? (
                <img src={images[activeImage]} alt={service.title} loading="lazy" className="h-full w-full object-cover" />
              ) : (
                <div className="flex h-full w-full items-center justify-center bg-neutral-100">
                  <span className="text-3xl font-bold text-neutral-300">{service.title[0]}</span>
                </div>
              )}
                ...
                {images.map((img, i) => (
                  <button
                    ...
                    className={cn(
                      ...
                      i === activeImage ? "border-neutral-900" : "border-transparent opacity-70 hover:opacity-100"
                    )}
                  >
            ...
            {service.video && (
              <a
                ...
                className="flex items-center gap-2 rounded-xl border border-neutral-200 px-3 py-2.5 text-[12.5px] font-medium text-neutral-600 transition-colors hover:border-neutral-400"
              >
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-neutral-100 text-neutral-900">
          ...
          <div className="flex min-w-0 flex-col">
            ...
            <p className="mt-1 line-clamp-2 max-w-2xl text-md leading-relaxed text-neutral-500">{service.description}</p>
            <div className="mt-1 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-neutral-100 pt-4 text-sm">
            ...
            {visibleSkills.length > 0 && (
              <div className="mt-2 border-t border-neutral-100 pt-2">
                <p className="mb-1 text-xs font-semibold text-neutral-400">Skills:</p>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="mt-1 grid grid-cols-1 items-center gap-4 border-t border-neutral-100 pt-4 sm:grid-cols-[1fr_auto_auto]">
              {freelancer && (
                <div className="flex min-w-0 items-center gap-3">
                  <div className="relative shrink-0">
                    {/* Same decorative conic-gradient ring used on the
                        influencer/freelancer cards, kept purely as an
                        accent. */}
                    <div
                      className="h-[52px] w-[52px] rounded-full p-[2px]"
                  ...
                  <div className="min-w-0">
                    <p className="flex items-center gap-1 truncate text-sm font-bold text-neutral-900">
                      {freelancer.name}
                      <BadgeCheck className="h-4 w-4 shrink-0 text-neutral-900" />
                    </p>
                    ...
                    {freelancer.level && (
                      <span className="mt-1 inline-flex rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] font-semibold text-neutral-600">
              ...
              <div className="flex gap-5 sm:gap-6">
                ...
                <span className="flex flex-col items-center">
                  <span className="flex items-center gap-1 text-base font-bold text-neutral-900">
                    <ThumbsUp className="h-4 w-4 text-neutral-900" />
              ...
              <div className="flex flex-col items-start gap-2 sm:items-end">
                ...
                <div className="flex w-full gap-2 sm:w-auto">
                  <span className="flex h-10 flex-1 items-center justify-center gap-1.5 rounded-full bg-neutral-900 px-4 text-sm font-semibold text-white transition-transform group-hover:scale-[1.02] sm:flex-none">
