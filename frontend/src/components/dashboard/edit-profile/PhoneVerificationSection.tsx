...
// Which provider is active decides which flow renders — Twilio is a plain
// send-code/check-code round trip through our own backend (real SMS now,
// same shape as the old email-OTP flow it replaced); Firebase's SMS send
// happens entirely client-side via invisible reCAPTCHA, and only the
// resulting ID token ever reaches the backend, to be verified there. See
// backend/src/modules/shared/user.controller.js for both server sides.
export function PhoneVerificationSection({ user }: { user: User }) {
  ...
  const sendTwilioOtp = useMutation({
    mutationFn: () => userApi.sendPhoneOtp(),
    onSuccess: () => setOtpSent(true),
  });
  const verifyTwilioOtp = useMutation({
    mutationFn: () => userApi.verifyPhoneOtp(otpInput),
    onSuccess: () => refreshUser(),
  });

  const sendFirebaseOtp = useMutation({
    mutationFn: async () => {
  ...
  const verifyFirebaseOtp = useMutation({
    mutationFn: async () => {
  ...
  const sendMutation = isFirebase ? sendFirebaseOtp : sendTwilioOtp;
  const verifyMutation = isFirebase ? verifyFirebaseOtp : verifyTwilioOtp;
